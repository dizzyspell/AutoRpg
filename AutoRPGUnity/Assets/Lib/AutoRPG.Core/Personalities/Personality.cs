...
namespace AutoRPG.Core.Personalities
{
    ...
    public abstract class Personality : IPersonality
    {
        ...
        {
            var fChanceSet = new List<Tuple<IAction, int>>
            {
            ...
            var fRandInt = RandomNumberGod.ChooseInt(mrTotalChance);

            IAction fChosenAction = null;

            foreach (var _action in fChanceSet)
            {
        ...
        {
            var fValidTargets = aAction.ValidTargets(Context);
